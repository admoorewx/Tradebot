...
def add_stock_from_json(json_file):
    ...
    with open(json_file) as json_file:
        ...
        for i,stock in enumerate(stocks):
            add_command = f'INSERT INTO stocks VALUES("{i}","{stock}","0","Pass","01/01/2022 00:00","0.00","0.00","0.00","initial");'
...
def get_stock_info(stock):
    connection, cursor = database()
    cursor.execute(f'SELECT symbol, owned, last_transaction, last_transaction_time, bought_price, sold_price, net, transaction_reason FROM stocks WHERE symbol = "{stock}";')
    response = cursor.fetchone()
    connection.close()
    return response

def update_stock(stock,owned,transaction,transaction_time,price,reason):
    connection, cursor = database()
    ...
    if transaction == "BUY":
        ...
        update_bought_price = f'UPDATE stocks SET bought_price = "{price}" WHERE symbol="{stock}";'
        update_sold_price = f'UPDATE stocks SET sold_price = "0.00" WHERE symbol="{stock}";'
        update_net = f'UPDATE stocks SET net = "0.00" WHERE symbol="{stock}";'
        commands = [update_owned, update_transaction, update_transaction_time, update_bought_price, update_sold_price, update_net, update_reason]
    elif transaction == "SELL":
        cursor.execute(f'SELECT bought_price FROM stocks WHERE symbol = "{stock}";')
        bp = cursor.fetchone()[0]
        net = price - bp
        update_transaction_time = f'UPDATE stocks SET last_transaction_time = "{transaction_time}" WHERE symbol="{stock}";'
        update_sold_price = f'UPDATE stocks SET sold_price = "{price}" WHERE symbol="{stock}";'
        update_net = f'UPDATE stocks SET net = "{net}" WHERE symbol="{stock}";'
        commands = [update_owned, update_transaction, update_transaction_time, update_sold_price, update_net, update_reason]
    else:
        commands = [update_owned, update_transaction, update_reason]
...
def create_report(email=False):
    ...
    lines.append("--------------------------------------\n")
    for stock in stocks:
        line = "{0:{9}}   {1:{10}}   {2}   {3:{11}}   {4}   {5:{12}}   {6:{12}}   {7:{13}}   {8}\n".format(stock[0],stock[1],stock[2],stock[3],stock[4],
                                                                                                            stock[5],stock[6],stock[7],stock[8],2, 5, 4, 5, 6)
        lines.append(line)
    # Email, if requested.
    if email:
        message = ""
